...
(file_names <- list.files(dir, pattern = '*working.csv', full.names = TRUE))
  ...
  #pull out 2019 -- dod that one separately
  file_names <- file_names[!(grepl('2019',file_names))]
  ...
  #now do 2019
  file2019 <- fread('clip_fire/review_fire_nwr2019_working.csv')
  file2019 <- file2019 %>% mutate(site_check = site) %>% select(site_check, date, fire) #I added one here manually so it will have 1 extra row compared to prev output
  
  #and combine again
  reviewed <- rbind(reviewed, file2019, fill = TRUE)
  
  #QC
  table(reviewed$fire, useNA = 'a')

  #summarize (all reviewed)
  n_distinct(reviewed$site_check, reviewed$date) #2133 distinct site-days
  site_day_combos <- reviewed %>% group_by(site_check) %>% summarise(n_days = length(unique(date))) 
  sum(site_day_combos$n_days) #same result (1489)
  summary(site_day_combos$n_days); sd(site_day_combos$n_days) #mean 10 (SD 12), range 1-90
  
  
# Summarize and format  --------------------------------------------------------
fires <- reviewed %>% select(site_check, date, fire) %>% filter(fire == 1) %>% group_by(site_check) %>%
  ...
  #summarize again (confirmed fires)
  nrow(fire_df) #confirmed fire on 141 unique camera-days
  n_distinct(fire_df$site, fire_df$date) #same, 141
  
  fire_summary <- fire_df %>% group_by(site) %>% 
                              mutate(year = year(date)) %>%
                              summarise(n_days = length(unique(date)),
                                        n_years = length(unique(year))) %>%
                              separate(site, c('hex','camera'), remove = FALSE)
  fire_summary
  summary(fire_summary$n_days)  #mean 1.3 days, range 1-4
  summary(fire_summary$n_years) #mean 1.2 days, range 1-4
  hist(fire_summary$n_days)
  hist(fire_summary$n_years)
  table(fire_summary$n_years) #1 year: 89, 2 years: 19, 3 years: 2, 4 years: 1
  
  length(unique(fire_summary$site)) #fires at 111 cameras
  length(unique(fire_summary$hex)) #from 61 hexagons
  
  length(unique(fire_df$date)) #on 102 different days
  length(fire_df$date) #total 141 days, so some were the same
  
  #months with fire?
  fire_df$month <- month(fire_df$date)
  
  table(fire_df$month)
  round(prop.table(table(fire_df$month)),2) #89% in Sept, Oct, Nov
    sum(c(0.29,0.37,0.23))
  hist(fire_df$month)
  
  #save
  write.csv(fire_df, 'clip_fire/fire_detections.csv')


  
# ------------------------------------------------------------------------------  
  
#### THE BELOW IS ONLY FOR THE OCCUPANCY MODEL VERSION OF THS ANALYSIS
  #### FOR GLMS AND PLOTS, PROCEED TO SCRIPT '03_fire_analyis.R'
