...
## NWR_2019----------------------------------------------
  ...
  folder_list <- fread('clip_fire/nwr2019_subfolders.txt', header = FALSE, sep = ',')
  
  #remove 'forms'
  folder_list <- folder_list$V1
  folder_list <- folder_list[!grepl('forms', folder_list, ignore.case = TRUE)]
  
  n_batches <- 3 #how many gpus
  (per_batch <- length(folder_list) / n_batches)
  
  cmd_list <- NULL
  for (ff in folder_list){
    #site <- sapply(strsplit(ff, '\\/'), '[', 8)
    index_ff <- which(folder_list == ff)
    gpu_ff <- ifelse(index_ff < per_batch, '0',  #if 3 GPUs
                     ifelse(index_ff < per_batch*2, '1', '2'))
    cmd_ff <- paste('CUDA_VISIBLE_DEVICES=', gpu_ff,
                    ' python',
  ...
  #save
  write.table(cmd_list[grepl('CUDA_VISIBLE_DEVICES=0', cmd_list)], col.names = FALSE, row.names = FALSE, quote = FALSE,
              file = 'clip_fire/batch_commands_clip_fire_nwr2019_gpu0.txt')
  write.table(cmd_list[grepl('CUDA_VISIBLE_DEVICES=1', cmd_list)], col.names = FALSE, row.names = FALSE, quote = FALSE,
              file = 'clip_fire/batch_commands_clip_fire_nwr2019_gpu1.txt')
  write.table(cmd_list[grepl('CUDA_VISIBLE_DEVICES=2', cmd_list)], col.names = FALSE, row.names = FALSE, quote = FALSE,
              file = 'clip_fire/batch_commands_clip_fire_nwr2019_gpu2.txt')  
  
  
## NWR_2020 ----------------------------------------------
  
  folder_list <- fread('clip_fire/nwr2020_subfolders.txt', header = FALSE)
  ...
  #save
  write.table(cmd_list[grepl('CUDA_VISIBLE_DEVICES=0', cmd_list)], col.names = FALSE, row.names = FALSE, quote = FALSE,
              file = 'clip_fire/batch_commands_clip_fire_nwr2020_gpu0.txt')
  write.table(cmd_list[grepl('CUDA_VISIBLE_DEVICES=1', cmd_list)], col.names = FALSE, row.names = FALSE, quote = FALSE,
              file = 'clip_fire/batch_commands_clip_fire_nwr2020_gpu1.txt')
  write.table(cmd_list[grepl('CUDA_VISIBLE_DEVICES=2', cmd_list)], col.names = FALSE, row.names = FALSE, quote = FALSE,
              file = 'clip_fire/batch_commands_clip_fire_nwr2020_gpu2.txt')
