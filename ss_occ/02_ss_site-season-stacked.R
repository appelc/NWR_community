...
## READ IN SITE-LEVEL COVARIATES -----------------------------------------------  ** NOT STANDARDIZED **
  ...
  site_covar <- fread('data/cleaned/covariates/cleaned_covariates.csv')
    ...
    head(site_covar[,..covar_names]) #these are NOT standardized 
...
## MAKE DETECTION COVARIATE FOR SEASON -----------------------------------------
  ...
  season_covar <- site_season_names %>% 
    ...
    column_to_rownames(var = 'site_season') %>%
...
## READ IN SURVEY-LEVEL EFFORT COVARIATE ---------------------------------------
  ...
  effort_covar <- effort_covar %>% arrange(site_primary) %>%
                                   column_to_rownames(var = 'site_primary') %>%
                                   select(-c(V1, fence, class))
...
## CREATE MODEL INPUTS ---------------------------------------------------------
  ...
  for (aa in names(dh_stacked)){
    ...
    occu_input_aa <- unmarkedFrameOccu(y = as.matrix(dh_aa),
                                       obsCovs = list(effort = effort_covar, 
                                                      season = season_covar, 
                                                      site_season = matrix(rownames(effort_covar), 
                                                                           nrow = length(rownames(effort_covar)), ncol = ncol(effort_covar))),
  ...
  doc0 <- doc0 %>%
    ...
    body_add_par(value = '')
  print(doc0, target = paste('ss_occ/outputs', 'naive_occ_target_sp.docx', sep = '/'))
...
## RUN DETECTION MODELS --------------------------------------------------------
  ...
  for (bb in focal_species){
    ...
    p_effort <- occu(formula = ~scale(effort) ~1, data = occu_input_bb)
    p_season <- occu(formula = ~season ~1, data = occu_input_bb)
    p_effort_season <- occu(formula = ~scale(effort) + season ~1, data = occu_input_bb)
...
## Store top detection models for each species ---------------------------------
  ...
  p_top_mods[['buffalo']] <- c(formula('~scale(effort) ~ 1'),
                               formula('~scale(effort) + season ~ 1'))
    
  p_aic_tables$bushbuck
  p_top_mods[['bushbuck']] <- c(formula('~scale(effort) ~ 1'),
                                formula('~scale(effort) + season ~ 1'))
  
  p_aic_tables$bushpig
  p_top_mods[['bushpig']] <- c(formula('~scale(effort) + season ~ 1'))
  
  p_aic_tables$eland
  p_top_mods[['eland']] <- c(formula('~scale(effort) + season ~ 1'),
                             formula('~season ~ 1'),
                             formula('~scale(effort) ~ 1'),
                             formula('~1 ~ 1'))
  
  p_aic_tables$elephant
  p_top_mods[['elephant']] <- c(formula('~scale(effort) + season ~ 1'))
  ...
  p_aic_tables$kudu
  p_top_mods[['kudu']] <- c(formula('~scale(effort) ~ 1'),
                            formula('~scale(effort) + season ~ 1'))
  
  p_aic_tables$reedbuck
  p_top_mods[['reedbuck']] <- c(formula('~scale(effort) + season ~ 1'),
                                formula('~scale(effort) ~ 1'))
  ...
  p_aic_tables$sable
  p_top_mods[['sable']] <- c(formula('~scale(effort) + season ~ 1'))
  
  p_aic_tables$warthog
  p_top_mods[['warthog']] <- c(formula('~scale(effort) ~ 1'),
                               formula('~scale(effort) + season ~ 1'))
  
  p_aic_tables$waterbuck
  p_top_mods[['waterbuck']] <- c(formula('~scale(effort) ~ 1'),
                                 formula('~scale(effort) + season ~ 1'))
  
  p_aic_tables$zebra
  p_top_mods[['zebra']] <- c(formula('~scale(effort) ~ 1'),
                             formula('~scale(effort) + season ~ 1'))
...
## RUN UNIVARIATE PSI MODELS (ENVIRONMENTAL VARS) ------------------------------
  ...
  (univar_psi <- lapply(c(1, envir_vars), function(x) as.formula(paste0('~1 ~scale(', x, ')')))) #include 1 for dot model too
  names(univar_psi) <- c('null', envir_vars)
  univar_psi$null <- as.formula('~1 ~ 1')
  ...
  for (hh in focal_species){
    ...
    univar_models_coef_psi <- univar_models_coef %>% filter(param == 'scale', variable != 'Int')
...
## Store top psi covariates for each species -----------------------------------
  ...
  psi_top_covars[['impala']] <- c('elev_mean_3x3')
  ...
  psi_top_covars[['roan']] <- c('prop_M3_3x3')
  
  psi_univar_aic_tables$sable
  psi_top_covars[['sable']] <- c('elev_mean_3x3')
  
  psi_univar_aic_tables$warthog
  psi_top_covars[['warthog']] <- c('elev_sd_3x3') #or just keep 'elev_sd_3x3'
...
## CONSTRUCT MULTIVARIATE PSI MODELS (ENVIR VARS) ------------------------------
  ...
  psi_top_covars <- readRDS('ss_occ/outputs/top_covars_psi.RDS')
  ...
  for (kk in names(psi_covars_multi)){
    ...
    #turn into formulas
    covar_combo_forms <- lapply(filtered_combos, function(covars_kk) {as.formula(paste0('~1 ~', paste('scale(', covars_kk, ')', collapse = " + ", sep = '')))})
...
## RUN MULTIVARIATE PSI MODELS (ENVIR VARS) ------------------------------------
  ...
  #run if necessary:
  occu_inputs <- readRDS('ss_occ/occu_inputs.RDS')
  multivar_combos <- readRDS('ss_occ/outputs/cand_models_multivar.RDS')
  ...
  saveRDS(psi_multivar_aic_tables, 'ss_occ/outputs/aic_tables_psi_multivariate.RDS')
  
  
## Construct multivar models with structure variables --------------------------

  ## Read in
  psi_multivar_aic_tables <- readRDS('ss_occ/outputs/aic_tables_psi_multivariate.RDS')
  psi_top_covars <- readRDS('ss_occ/outputs/top_covars_psi.RDS')
  ...
  ## Create lists of top models *so far* for each species
    ...
    names(psi_top_covars[sapply(psi_top_covars, length) == 1])
      ...
      psi_top_multi[['impala']] <- gsub('~1 ~ ', '', psi_top_covars$impala)
      psi_top_multi[['kudu']] <- gsub('~1 ~ ', '', psi_top_covars$kudu)
      psi_top_multi[['reedbuck']] <- gsub('~1 ~ ', '', psi_top_covars$reedbuck)
      psi_top_multi[['roan']] <- gsub('~1 ~ ', '', psi_top_covars$roan)
      psi_top_multi[['sable']] <- gsub('~1 ~ ', '', psi_top_covars$sable)
      psi_top_multi[['warthog']] <- gsub('~1 ~ ', '', psi_top_covars$warthog)
  ...
  str_model_list <- list()
    ...
    for (qq in sort(names(psi_top_multi))) {
      ...
      for (rr in 1:length(mods_qq)){ 
        ...
        #separate covariates if in equation form
        rr_covars <- strsplit(mods_qq[rr], '\\+')[[1]]
        
        #for each top uni/multivar combo, add this model as-is
        str_model_list[[qq]] <- append(str_model_list[[qq]], formula(paste0('~1 ~', paste0('scale(', rr_covars, ')', collapse = ' + '))))
        
        #add 'season' and 'fence*period to this model
        str_model_list[[qq]] <- append(str_model_list[[qq]], formula(paste0('~1 ~', paste0('scale(', rr_covars, ')', collapse = ' + '), ' + season')))
        str_model_list[[qq]] <- append(str_model_list[[qq]], formula(paste0('~1 ~', paste0('scale(', rr_covars, ')', collapse = ' + '), ' + fence*period')))
        str_model_list[[qq]] <- append(str_model_list[[qq]], formula(paste0('~1 ~', paste0('scale(', rr_covars, ')', collapse = ' + '), ' + season + fence*period')))
        
        #add 'releases' to this model (*if* the species had supplemental releases)
        if(qq %in% sp_suppl) {
          # str_model_list[[qq]] <- append(str_model_list[[qq]], formula(paste0('~1 ~', paste0('scale(', rr_covars, ')', collapse = ' + '), ' + releases')))
          # str_model_list[[qq]] <- append(str_model_list[[qq]], formula(paste0('~1 ~', paste0('scale(', rr_covars, ')', collapse = ' + '), ' + releases + fence*period')))
          str_model_list[[qq]] <- append(str_model_list[[qq]], formula(paste0('~1 ~', paste0('scale(', rr_covars, ')', collapse = ' + '), ' + fence*releases')))
          str_model_list[[qq]] <- append(str_model_list[[qq]], formula(paste0('~1 ~', paste0('scale(', rr_covars, ')', collapse = ' + '), ' + season + fence*releases')))
        }
      } #end top models loop
    } #end species loop
    
      #inspect and save
      lapply(str_model_list, length) #ones with univariate and no releases should have 2/4, ones with univariate + releases should have 4/6, multivariate should have 4/6*n_models
      str_model_list
...
## RUN MULTIVARIATE PSI MODELS (ENVIR + STRUCTURAL) -----------------------------
  ...
  #read in if necessary
  str_model_list <- readRDS('ss_occ/outputs/cand_models_structure.RDS')
  occu_inputs <- readRDS('ss_occ/occu_inputs.RDS')
  ...
  ## no errors about convergence, good!
...
## Now construct final candidate models ----------------------------------------
  ...
  psi_top_mods <- list()
    ...
    psi_top_mods[['buffalo']] <- c(formula('~1 ~ scale(elev_sd_3x3) + scale(pct_slope_sd_3x3) + fence * releases'),
                                   formula('~1 ~ scale(elev_sd_3x3) + scale(pct_slope_sd_3x3) + fence * period'),
                                   formula('~1 ~ scale(elev_sd_3x3) + scale(pct_slope_sd_3x3) + season + fence * period'))
    
    psi_struct_aic_tables$bushbuck
    psi_top_mods[['bushbuck']] <- c(formula('~1 ~ scale(tree_vol_mean_3x3) + fence * period'),
                                    formula('~1 ~ scale(tree_vol_mean_3x3) + season + fence * period'))
    
    psi_struct_aic_tables$bushpig
    psi_top_mods[['bushpig']] <- c(formula('~1 ~ scale(tree_vol_mean_3x3) + season + fence * period'))

    psi_struct_aic_tables$eland
    psi_top_mods[['eland']] <- c(formula('~1 ~ scale(dist_cleared_sd_3x3) + fence * releases'),
                                 formula('~1 ~ scale(dist_cleared_sd_3x3) + season + fence * period'))

    psi_struct_aic_tables$elephant
    psi_top_mods[['elephant']] <- c(formula('~1 ~ scale(prop_M3_3x3) + season + fence * period'))
    
    psi_struct_aic_tables$impala
    psi_top_mods[['impala']] <- c(formula('~1 ~ scale(elev_mean_3x3) + season + fence * releases'),
                                  formula('~1 ~ scale(elev_mean_3x3) + fence * period'),
                                  formula('~1 ~ scale(elev_mean_3x3) + season + fence * period'))
                                  
    psi_struct_aic_tables$kudu
    psi_top_mods[['kudu']] <- c(formula('~1 ~ scale(elev_mean_3x3) + season + fence * period'),
                                formula('~1 ~ scale(elev_mean_3x3) + fence * period'),
                                formula('~1 ~ scale(elev_mean_3x3) + season + fence * releases'))
    
    psi_struct_aic_tables$reedbuck
    psi_top_mods[['reedbuck']] <- c(formula('~1 ~ scale(dist_cleared_mean_3x3) + fence * period'),
                                    formula('~1 ~ scale(dist_cleared_mean_3x3) + season + fence * period'))
    
    psi_struct_aic_tables$roan
    psi_top_mods[['roan']] <- c(formula('~1 ~ scale(prop_M3_3x3) + fence * period'),
                                formula('~1 ~ scale(prop_M3_3x3)'),
                                formula('~1 ~ scale(prop_M3_3x3) + season + fence * period'))
    
    psi_struct_aic_tables$sable
    psi_top_mods[['sable']] <- c(formula('~1 ~ scale(elev_mean_3x3) + fence * releases'),
                                 formula('~1 ~ scale(elev_mean_3x3) + season + fence * releases'),
                                 formula('~1 ~ scale(elev_mean_3x3) + fence * period'))
    
    psi_struct_aic_tables$warthog
    psi_top_mods[['warthog']] <- c(formula('~1 ~ scale(elev_sd_3x3) + season + fence * period'),
                                   formula('~1 ~ scale(elev_sd_3x3) + season + fence * releases'))
    
    psi_struct_aic_tables$waterbuck
    psi_top_mods[['waterbuck']] <- c(formula('~1 ~ scale(elev_mean_3x3) + season'), 
                                     formula('~1 ~ scale(elev_mean_3x3)'),
                                     formula('~1 ~ scale(elev_mean_3x3) + season + fence * period'),
                                     formula('~1 ~ scale(elev_mean_3x3) + season + fence * releases'),
                                     formula('~1 ~ scale(elev_mean_3x3) + fence * period'))
    
    psi_struct_aic_tables$zebra
    psi_top_mods[['zebra']] <- c(formula('~1 ~ scale(dist_cleared_mean_3x3) + scale(tree_vol_mean_3x3) + fence * releases'),
                                 formula('~1 ~ scale(dist_cleared_mean_3x3) + scale(prop_M1_3x3) + fence * releases'),
                                 formula('~1 ~ scale(dist_cleared_mean_3x3) + scale(tree_vol_mean_3x3) + fence * period'),
                                 formula('~1 ~ scale(dist_cleared_mean_3x3) + scale(prop_M1_3x3) + season + fence * releases'),
                                 formula('~1 ~ scale(dist_cleared_mean_3x3) + scale(tree_vol_mean_3x3) + season + fence * period'))
...
## RUN FINAL MODELS  -----------------------------------------------------------
  ...
  top_aic_tables_print <- top_aic_tables_paper %>% mutate(across(c(AIC, delta, AICwt, cumltvWt), ~ round(.x, 2))) %>%  
                                                   flextable() %>% 
                                                   colformat_num(big.mark = '') %>%
                                                   bg(which(is.na(top_aic_tables_paper$nPars)), bg = "darkseagreen3") %>%
                                                   autofit()
  doc6 <- read_docx()
