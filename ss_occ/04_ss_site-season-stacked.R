...
## READ IN SITE-LEVEL COVARIATES -----------------------------------------------  
  ...
  #change name of 'dist_human' **TEMPORARY... FIX IN COVAR SCRIPT**
  site_covar_expanded$dist_cleared_mean_3x3 <- site_covar_expanded$dist_human_mean_3x3
  site_covar_expanded$dist_cleared_sd_3x3 <- site_covar_expanded$dist_human_sd_3x3
  site_covar_expanded$dist_cleared_mean_5x5 <- site_covar_expanded$dist_human_mean_5x5
  site_covar_expanded$dist_cleared_sd_5x5 <- site_covar_expanded$dist_human_sd_5x5
...
## CREATE MODEL INPUTS ---------------------------------------------------------
  ...
  for (ss in names(dh_stacked)){
    ...
    #add field for before/after fence decommissioning
...
## RUN DETECTION MODELS --------------------------------------------------------
  ...
  for (pp in focal_species){
    ...
    p_aic_tables[[pp]] <- p_fit_list %>% select(model, nPars, AIC, delta, AICwt, cumltvWt) %>% #add betas here??
  ...
  for(bb in focal_species){
    doc1 <- doc1 %>% 
      body_add_flextable(p_aic_tables[[bb]]) %>%
      body_add_par(value = '')
  }
...
## RUN UNIVARIATE PSI MODELS (ENVIRONMENTAL VARS) ------------------------------
  ...
  ## PULL THIS OUTSIDE THE LOOP OR ADD TO OCC MOD FUNCTIONS DOC
    #function to add coef and SE to mod sel table

    #also try:
    coef(univar_model_list)
    SE(univar_model_list)
    
  ##  
    
    #save formatted AIC table
    psi_univar_aic_tables[[mm]] <- univar_fit_list %>% select(model, nPars, AIC, delta, AICwt) %>%
                                                            mutate(across(c(AIC, delta, AICwt), ~ round(.x, 2))) %>%  
                                                            flextable() %>% 
                                                            colformat_num(big.mark = '') %>%
    ...
    #store models
    psi_univar_models[[mm]] <- univar_models_mm
  }
...
## RUN MULTIVARIATE PSI MODELS -------------------------------------------------
  ...
  for (yy in names(multivar_combos)){
    ...
    #save formatted AIC table
    psi_multivar_aic_tables[[yy]] <- multivar_fit_list %>% select(model, nPars, AIC, delta, AICwt, cumltvWt, Rsq) %>%
...
## ADD STRUCTURAL VARS + INTERACTION -------------------------------------------
  ...
  for (vv in names(model_lists)){
    ...
    interact_fit_list <- as(modSel(interact_model_list), 'data.frame')
    
    #save formatted AIC table
    psi_interact_aic_tables[[vv]] <- interact_fit_list %>% select(model, nPars, AIC, delta, AICwt, cumltvWt, Rsq) %>%
      mutate(across(c(AIC, delta, AICwt, cumltvWt, Rsq), ~ round(.x, 2))) %>%  
      flextable() %>% 
      colformat_num(big.mark = '') %>%
      bg(i = ~ model == as.character('~1 ~ 1'), bg = "lightgray") %>%
      bg(which.max(interact_fit_list$nPars), bg = "lightblue") %>% #shade global model here too
      autofit() %>%
      set_caption(vv)
