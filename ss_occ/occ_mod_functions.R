...
## Function to create table with parameter estimates and confidence intervals
param_table <- function(model, conf_threshold, modname) {
  
  #modname <- as.character(substitute(model))
  
  table <- rbind(data.frame(confint(type = 'state', level = conf_threshold, model)),
                 data.frame(confint(type = 'det', level = conf_threshold, model)))
  table$beta <- coef(model)
  table$covar <- rownames(table); rownames(table) <- NULL
  table$modname <- rep(modname, nrow(table))
  colnames(table) <- c('LCI_95','UCI_95','beta','covar','mod_name')
  table <- table[,c('mod_name','covar','beta','LCI_95','UCI_95')]
...
occPred <- function(modname, new_data, nickname){ #nickname argument not working
  ...
  predLong <- reshape2::melt(predWide, id.vars = names(new_data), value.name = 'value')
  lciLong  <- reshape2::melt(lciWide, id.vars = names(new_data), value.name = 'LCI')
  uciLong  <- reshape2::melt(uciWide, id.vars = names(new_data), value.name = 'UCI')
